...
impl Tablet {
    ...
    pub fn new(dev: Device) -> Tablet {
        let mut tablet = Tablet {
            device: Device::new().unwrap(), // temp blank value
            name: String::new(),
            location: String::new(),
            uniq_id: String::new(),
            version: 0,
        ...
        tablet.location = dev.phys().unwrap_or("").to_string();
        tablet.uniq_id = dev.uniq().unwrap_or("").to_string();
        tablet.version = dev.version();
        tablet.driver_version = dev.driver_version();
        tablet.fill_properties(&dev);
        tablet.fill_events(&dev);
        tablet.device = dev; // The device is the last to move
        tablet
    }
